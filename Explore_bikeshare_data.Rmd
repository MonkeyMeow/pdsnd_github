...
**I did not use loops in some places because the print() function did not produce nice displays.**

```{r echo=FALSE}
n <- 1
city.names[[n]]
head(my.data.frames[[n]])
tail(my.data.frames[[n]])
```
...
city.names[[n]]
freq.dfs[[n]]
p %+% freq.dfs[[n]] + geom_point(colour = my.colours[[n]]) + ggtitle(paste(title.beg, city.names[[n]])) + my.themes[[n]]
```
...
n <- 1
by(my.data.frames[[n]]$DoW, my.data.frames[[n]]$User.Type, summary)
city.names[[n]]
p %+% my.data.frames[[n]] + geom_bar(fill = my.colours[[n]]) + my.themes[[n]] + ggtitle(paste(city.names[[n]], title.end))
```
...
n <- 1
city.names[[n]]
summary(my.data.frames[[n]]$Trip.Minutes)
by(my.data.frames[[n]]$Trip.Minutes, my.data.frames[[n]]$User.Type, summary)
p %+% my.data.frames[[n]] + geom_histogram(binwidth = 1, fill = my.colours[[n]]) +
  my.themes[[n]]  + ggtitle(paste(city.names[[n]], title.end))
```
...
by(df$Birth.Year, df$Gender, summary)
p %+% df + geom_histogram(binwidth = 1, fill = my.colours[[n]]) +
  my.themes[[n]] + ggtitle(paste(city.names[[n]], title.end))
```
